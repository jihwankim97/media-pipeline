...
export class AuthService {
  ...
  async parseBearerToken(rawToken: string, isRefreshToken: boolean) {
    ...
    try {
      ...
      if (isRefreshToken) {
        ...
        if (payload.type === 'refresh') {
          throw new BadRequestException('Acess입력해주세요');
        }
      }
      return payload;
    } catch (error) {
  ...
  async issueToken(user: { id: number; role: Role }, isRefreshToken: boolean) {
    const refreshTockenSecret = this.configService.get<string>(
      envVariableKeys.refreshTokenSecret,
    );

    const accessTockenSecret = this.configService.get<string>(
    ...
    return await this.jwtService.signAsync(
      ...
      {
        secret: isRefreshToken ? refreshTockenSecret : accessTockenSecret,
